...
angular
  ...
  .controller("AppController", [
    ...
    function (
      ...
      self.fnObjKeys = function (obj) {
        return Object.keys(obj);
      };
      self.fnObjValues = function (obj) {
      ...
      function InitializeSessionIfNecessary() {
        if (null != _SessionIDToken) return;
        ...
        $http.get(_APIBaseURL + "InitSession", config).then(
          function (result) {
        ...
        $http(config).then(
          function (result) {
            ...
            document.getElementById(
              "ifDetails"
            ).contentDocument.body.innerHTML = JSON.stringify(result.data);
      ...
      function SuggestedNextStepsController(
        $scope,
        $mdDialog,
        MyMainController
      ) {
        ...
        function initialize() {
          MyMainController.DoGetSuggestedFeatures_PatientProvided().then(
            function (result) {
              self.SuggestedFeatures_PatientProvided = result;
            }
          );

          MyMainController.DoGetSuggestedFeatures_PhysicianProvided().then(
            function (result) {
              self.SuggestedFeatures_PhysicianProvided = result;
            }
          );

          MyMainController.DoGetSuggestedFeatures_Tests().then(function (
            result
          ) {
            self.SuggestedFeatures_Tests = result;
          });

          MyMainController.DoGetSuggestedSpecializations().then(function (
            result
          ) {
            self.SuggestedSpecializations = result;
          });
        }
      ...
      function DisplaySuggestedNextStepsDialog(symptom) {
        ...
        $mdDialog.show(dlgNextSteps).then(function (answer) {
          // alert("confirmed: " + answer);
        });
      }
      ...
      $scope.fnSetUseDefaultValues = function () {
        ...
        $http
          .post(
            _APIBaseURL +
              "SetUseDefaultValues?SessionID=" +
              _SessionIDToken +
              "&value=" +
              self.bUseDefaultValues,
            "",
            config
          )
          .then(
      ...
      function DisplayAPIResponse(result) {
        $mdToast.show(
          $mdToast
            .simple()
            .textContent(
              "Received Http Response: " + JSON.stringify(result.data)
            )
            .position("bottom right")
            .hideDelay(5000)
        );
  ...
  .controller("RightSideNavCtrl", function (
    $scope,
    $timeout,
    $mdSidenav,
    $log,
    ...
    $scope.AddSymptom = function () {
      if (self.selectedItemCopy.bIsCreated == true) {
        ...
        $scope.$parent.ctrl.usersymptoms[
          self.selectedItemCopy.listSymptomsIndex
        ].value = self.selectedItemCopy.value;
      } else {
        // sprawdzamy czy juz przypadkiem taki symptom nie istnieje:
        var iArrLen = $scope.$parent.ctrl.usersymptoms.length,
          kk;
        var bItemAlreadyFound = false;

        for (kk = 0; kk < iArrLen; kk++) {
          if (
            $scope.$parent.ctrl.usersymptoms[kk].name ==
            self.selectedItemCopy.name
          ) {
        ...
        if (false == bItemAlreadyFound) {
          ...
          self.selectedItemCopy.listSymptomsIndex =
            $scope.$parent.ctrl.usersymptoms.length;
      ...
      try {
        $scope.$parent.ctrl.UpdateSymptomValue(
          angular.copy(self.selectedItemCopy)
        );
      } catch (e) {
    ...
    function loadSymptoms() {
      ...
      return $http
        .get("../SymptomsOutput.json") //'symptoms.json')
        .then(function (res) {
    ...
    function querySearchSymptoms(query) {
      ...
      var results = query
        ? arrSymptoms.filter(createFilterFor(query))
        : arrSymptoms; //,
      //deferred;
    ...
    function createFilterFor(query) {
      ...
      return function filterFn(symptom) {
        ...
        return (
          ...
          (symptom.alias &&
            angular.lowercase(symptom.alias).indexOf(lowercaseQuery) >= 0) ||
          (symptom.choices &&
            symptom.choices.some(function (elem, idx) {
              return (
                elem.humanlang &&
                angular.lowercase(elem.humanlang).indexOf(lowercaseQuery) >= 0
              );
            }))
        );
      };
